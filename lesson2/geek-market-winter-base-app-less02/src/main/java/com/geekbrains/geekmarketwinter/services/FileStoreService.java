...
public class FileStoreService implements IFileStoreService {
    ...
    public void deleteFile(UUID md5, String filename) throws IOException {
        String file = fileMetaProvider.checkFileNameExists(md5, filename);

        String ext = FilenameUtils.getExtension(file);
        String fullFileName = md5.toString() + "." + ext;
        fileMetaProvider.deleteFileMeta(md5, file);
    ...
    public String existsWithFileName(UUID fileHash, String fileName) {
        return  fileMetaProvider.checkFileNameExists(fileHash, fileName);
    }
}